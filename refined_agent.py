...
_HALT = actions.FUNCTIONS.Halt_quick.id
_HALT2 = actions.FUNCTIONS.Halt_Building_quick.id
_HALT3 = actions.FUNCTIONS.Halt_TerranBuild_quick.id
...
class QLearningTable:
    ...
    def choose_action(self, observation, excluded_actions=[]):
        ...
        #excluded_actions.sort(reverse=True)
...
class SparseAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        supply_depot_count = int(round(len(depot_y) / 69.0))

        barracks_y, barracks_x = (unit_type == _TERRAN_BARRACKS).nonzero()
        barracks_count = int(round(len(barracks_y) / 137.0))
